"""Environment-backed configuration for the EchoGuard sidecar."""
...
class Settings:
    ...
    audit_db: Path = Path("/data/echoguard.sqlite3")
    ...
    def from_env(cls) -> "Settings":
        ...
        raw = os.getenv("ECHOGUARD_MCP_UPSTREAMS", "").strip()
        if raw:
            parsed = json.loads(raw)
            if not isinstance(parsed, dict):
                raise ValueError("ECHOGUARD_MCP_UPSTREAMS must be a JSON object")
            mcp_upstreams.update({str(k): str(v) for k, v in parsed.items()})

        return cls(
            host=os.getenv("ECHOGUARD_HOST", "0.0.0.0"),
            port=int(os.getenv("ECHOGUARD_PORT", "8200")),
            agent_upstream=os.getenv("ECHOGUARD_AGENT_UPSTREAM", "http://opspilot-app:8000").rstrip("/"),
            llm_upstream=os.getenv("ECHOGUARD_LLM_UPSTREAM", "http://llm-stub:8000").rstrip("/"),
            langflow_upstream=os.getenv("ECHOGUARD_LANGFLOW_UPSTREAM", "http://langflow:7860").rstrip("/"),
            mcp_upstreams=mcp_upstreams,
            target_path=Path(os.getenv("ECHOGUARD_TARGET", "/target")),
            audit_db=Path(os.getenv("ECHOGUARD_AUDIT_DB", "/data/echoguard.sqlite3")),
            jwt_secret=os.getenv("ECHOGUARD_JWT_SECRET", "change-me-weak-secret"),
            request_timeout_s=float(os.getenv("ECHOGUARD_REQUEST_TIMEOUT", "15")),
            trace_ttl_s=int(os.getenv("ECHOGUARD_TRACE_TTL", "3600")),
        )
